...
if(!empty($q)){
  ...
  <div class="bodyblock">
    ...
    $related_st_id = [];
    $related_cnt = 0;
    $related_st_id = getrelatedstory($q, $related_st_id);           //notice that i only allow 100- results
    $related_st_id = getsimtitle($q, $related_st_id);           //notice that i only allow 100- results
...
function getsimtitle($keyword, $related){
	require 'databaselogin.php';
	if (!empty($keyword)){
        $keyword = '%'.$keyword.'%';
		$i = 0;
		$getrelated = $mysqli->prepare("select st_id from story where title like ? ORDER BY st_id DESC");
		$getrelated->bind_param('s', $keyword);
		$getrelated->execute();
		$getrelated->bind_result($related_st_id_db);
		while($getrelated->fetch() && $i <= 100){
			array_push($related, $related_st_id_db);
